...
<template>
  ...
  <p>
    {{ pollSiteInfo.name }}<br>
    {{ pollSiteInfo.address }}
  </p>
...
export default {
  ...
  ready: function() {
      ...
      this.$http.jsonp(this.url).then(function (response) {
          ...
          this.pollSiteInfo = {
            'address':this.APIresponse['site_address'],
            'name':this.APIresponse['site_name']
          };
            console.log(this.pollSiteInfo, 'Poll Site: Site address and name saved');
